  ...
  <div class="min-h-screen flex bg-slate-50">
    ...
    <aside id="nav-sidebar" :class="['fixed top-0 left-0 h-full bg-white border-r border-slate-200 z-40 flex flex-col transition-all duration-200 lg:translate-x-0 w-64', sidebarOpen ? 'translate-x-0' : '-translate-x-full', sidebarCollapsed ? 'lg:w-16' : '']">
      ...
      <div class="h-16 border-b border-slate-200 flex-shrink-0 flex items-center justify-between px-3">
        ...
        <button @click="sidebarCollapsed = !sidebarCollapsed" class="hidden lg:flex p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors flex-shrink-0" :title="sidebarCollapsed ? t('layout.expandSidebar') : t('layout.collapseSidebar')">
      ...
      <div class="p-2 border-t border-slate-200">
        <div class="flex items-center gap-2 px-1 py-2">
          ...
          <button v-show="!sidebarCollapsed || sidebarOpen" @click="auth.logout()" class="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors flex-shrink-0" :title="t('layout.logout')">
        ...
        <div v-show="!sidebarCollapsed || sidebarOpen" class="flex items-center gap-1.5 px-1 pb-1">
          ...
          <button @click="startTour(true)" class="p-1.5 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" :title="t('layout.repeatTour')">
    ...
    <div :class="['flex-1 flex flex-col transition-all duration-200', sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-64']">
      ...
      <header class="sticky top-0 z-20 h-16 bg-white border-b border-slate-200 flex items-center px-4 gap-4">
        ...
        <div v-if="pendingCount > 0" class="flex items-center gap-1.5 text-xs text-amber-600 bg-amber-50 border border-amber-200 px-2.5 py-1.5 rounded-lg">
          ...
          {{ pendingCount }} {{ t('layout.pending') }}
          <button @click="retrySync" class="underline hover:no-underline ml-0.5">{{ t('layout.retry') }}</button>
          <span class="text-amber-300">·</span>
          <button @click="clearQueue" class="underline hover:no-underline text-amber-500">{{ t('layout.clear') }}</button>
        </div>
        ...
        <div class="relative" ref="notifRef">
          ...
          <div v-if="notifOpen" class="absolute right-0 top-12 w-80 max-w-[calc(100vw-2rem)] bg-white border border-slate-200 rounded-xl shadow-lg z-50 overflow-hidden">
            <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
              <span class="font-semibold text-sm text-slate-900">{{ t('layout.notifications') }}</span>
              <button @click="notif.markRead(); notifOpen=false" class="text-xs text-primary-600 hover:underline">{{ t('layout.markAllRead') }}</button>
            </div>
            <div class="max-h-80 overflow-y-auto divide-y divide-slate-50">
              <div v-if="!notif.items.length" class="p-4 text-center text-sm text-slate-400">{{ t('layout.noNotifications') }}</div>
      ...
      <div v-if="tabs.pageTabs.length" class="sticky top-16 z-10 bg-slate-50 border-b border-slate-200 px-4 py-1.5 flex items-center gap-1 overflow-x-auto">
        <span class="text-[10px] text-slate-400 mr-2 flex-shrink-0 font-medium uppercase tracking-wider">{{ t('layout.pageTabs') }}</span>
        <div v-for="pt in tabs.pageTabs" :key="pt.path" :class="['page-tab-item group', route.path === pt.path && 'active']" @click="navigateTo(pt.path)">
          ...
          <button v-if="!pt.pinned" @click.stop="tabs.togglePagePin(pt.path)" class="opacity-100 lg:opacity-0 lg:group-hover:opacity-100 ml-0.5 p-0.5 rounded hover:bg-slate-300 transition-all" :title="t('layout.pinThisPage')">
          ...
          <button v-else @click.stop="tabs.togglePagePin(pt.path)" class="ml-0.5 p-0.5 rounded hover:bg-slate-300 transition-all" :title="t('layout.unpin')">
      ...
      <div v-if="tabs.tabs.length" class="sticky z-10 bg-white border-b border-slate-200 px-4 py-2 flex items-center gap-1.5 overflow-x-auto" :style="{ top: tabs.pageTabs.length ? '6rem' : '4rem' }">
        ...
        <div v-for="tab in tabs.tabs" :key="tab.id" :class="['tab-item group', tab.id === tabs.activeTabId && 'active']" @click="tabs.openTab(tab)">
          ...
          <button v-if="!tab.pinned" @click.stop="tabs.togglePin(tab.id)" class="opacity-100 lg:opacity-0 lg:group-hover:opacity-100 ml-0.5 p-0.5 rounded hover:bg-slate-200 transition-all" :title="t('layout.pin')">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"/></svg>
          </button>
          <button v-else @click.stop="tabs.togglePin(tab.id)" class="ml-0.5 p-0.5 rounded hover:bg-slate-200 transition-all" :title="t('layout.unpin')">
            <svg class="w-3 h-3 text-primary-500" fill="currentColor" viewBox="0 0 24 24"><path d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"/></svg>
          </button>
      ...
      <div v-if="tabs.qcTabs.length" class="sticky z-10 bg-amber-50 border-b border-amber-100 px-4 py-2 flex items-center gap-1.5 overflow-x-auto"
        ...
        <div v-for="tab in tabs.qcTabs" :key="tab.id"
          ...
          <button v-if="!tab.pinned" @click.stop="tabs.toggleQcPin(tab.id)" class="opacity-100 lg:opacity-0 lg:group-hover:opacity-100 ml-0.5 p-0.5 rounded hover:bg-amber-200 transition-all" :title="t('layout.pin')">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"/></svg>
          </button>
          <button v-else @click.stop="tabs.toggleQcPin(tab.id)" class="ml-0.5 p-0.5 rounded hover:bg-amber-200 transition-all" :title="t('layout.unpin')">
...
function fallbackMenus(role?: string) {
  const base: any[] = [
    ...
    { id: 'f4n', name: t('layout.notifications'), path: '/notifications', icon: 'bell', parent_id: null },
    { id: 'f5', name: t('layout.calendar'), path: '/calendar', icon: 'calendar', parent_id: null },
    { id: 'f-wishlist', name: t('layout.notes'), path: '/wishlist', icon: 'clipboard', parent_id: null },
    { id: 'f-qcforms', name: t('layout.qcForm'), path: '/qc-forms', icon: 'check-square', parent_id: null },
