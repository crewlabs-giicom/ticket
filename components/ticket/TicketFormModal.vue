      ...
      <form @submit.prevent="submit" class="p-6 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div><label class="label">Priority</label>
            <select v-model="form.priority_id" class="input" required>
              <option v-for="p in priorities" :key="p.id" :value="p.id">{{ p.name }}</option>
            </select>
          </div>
          <div><label class="label">Status</label>
            <select v-model="form.status_id" class="input" required>
...
const form = reactive({ title: '', description: '', project_id: '', priority_id: priorities.value[0]?.id || '', status_id: statuses.value[0]?.id || '', assigned_to: '' })
...
async function submit() {
  ...
  loading.value = true
  try {
    await $fetch('/api/tickets', { method: 'POST', body: { ...form, attachments: uploadedFiles.value } })
    emit('created')
  } catch (e: any) {
