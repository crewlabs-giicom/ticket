<template>
  <div class="w-full max-w-sm">
    <div class="card p-8">
      ...
      <h1 class="text-xl font-bold text-center text-slate-900 mb-1">{{ t('login.welcome') }}</h1>
      <p class="text-sm text-center text-slate-500 mb-6">{{ t('login.subtitle') }}</p>
      ...
      <div class="relative my-4">
        ...
        <div class="relative flex justify-center"><span class="bg-white px-2 text-xs text-slate-400">{{ t('login.or') }}</span></div>
      </div>
      ...
      <div v-if="showHrisForm" class="mt-3 p-4 bg-blue-50 rounded-lg border border-blue-200 space-y-3">
        <p class="text-xs text-blue-700 font-medium">{{ t('login.hrisTitle') }}</p>
        <div>
          <label class="label">{{ t('login.hrisEmail') }}</label>
          <input v-model="hrisForm.email" type="email" class="input" placeholder="email@giisystem.com" />
        </div>
        <div>
          <label class="label">{{ t('login.hrisPassword') }}</label>
        ...
        <button type="button" @click="handleHrisLogin" class="btn-primary w-full justify-center py-2.5" :disabled="hrisLoading">
          ...
          {{ hrisLoading ? t('login.hrisSubmitting') : t('login.hrisSubmit') }}
        </button>
      </div>

      <p class="text-center text-sm text-slate-500 mt-4">
        {{ t('login.noAccount') }}
        <NuxtLink to="/register" class="text-primary-600 hover:underline font-medium">{{ t('login.register') }}</NuxtLink>
      </p>
    </div>
  </div>
</template>
...
async function handleHrisLogin() {
  ...
  if (!hrisForm.email || !hrisForm.password) {
    hrisError.value = t('login.hrisRequired')
    return
  }
  ...
  try {
    ...
    hrisError.value = e?.data?.statusMessage || t('login.hrisFailed')
  } finally {
